...
class Recipes(commands.Cog):
    ...
    async def global_recipes(
        ...
        if len(recipes) == 0:
            await interaction.send("No hay recipes globales")
    ...
    async def add_recipe(
        ...
        if item_name in recipe_names:
            await interaction.send(f"*{item_name}* ya está registrado en la lista de recipes globales")
        else:
            add_recipe(item_name, item_level)
            await interaction.send(f"*{item_name}* se registró en la lista de recipes globales")
