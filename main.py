...
@gets_pj_data
async def registrar(interaction: nextcord.Interaction, nombre_pj:str, nombre_jugador:str):
...
@gets_pj_data
async def downtime(interaction: nextcord.Interaction, amount:float, user:nextcord.Member=None):
    ...
    return await interaction.send(f"{pj_name} {"gana" if amount>0 else "gasta"} {amount:.2f} semanas de downtime. Ahora tiene {new_total:.2f}")

@bot.slash_command(description="Resta dinero de tu cuenta. Puedes transferir a otra persona.", guild_ids=[CRI_GUILD_ID])
@gets_pj_data
async def pagar(interaction: nextcord.Interaction, amount: float, transfertarget:nextcord.Member= None):
    user_id:int = interaction.user.id
    ...
    if transfertarget is not None:
        ...
        return await interaction.send(f"{pj_name} le transfiere {amount}gp ({ppp}pp, {pgp}gp, {pep}ep, {psp}sp, {pcp}cp) a {target_pj_name}. \n{pj_name} queda con {new_total:.2f}gp, y {target_pj_name} queda con {new_total_target:.2f}gp.")
    return await interaction.send(f"{pj_name} paga {amount}gp ({ppp}pp, {pgp}gp, {pep}ep, {psp}sp, {pcp}cp).\nAhora tiene {pp}pp, {gp}gp, {ep}ep, {sp}sp, {cp}cp, **Total: {new_total:.2f}gp**")

@bot.slash_command(description="Suma dinero a tu cuenta.", guild_ids=[CRI_GUILD_ID])
@gets_pj_data
async def añadirdinero(interaction: nextcord.Interaction, amount: float, user:nextcord.Member= None):
    user_id:int = interaction.user.id if user == None else user.id
...
@gets_pj_data
async def completarmision(interaction: nextcord.Interaction, level:int, user:nextcord.Member=None):
    user_id:int = interaction.user.id if user == None else user.id
    pass
