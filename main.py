...
default_user_option = nextcord.SlashOption(
    name="usuario-target",
...
async def pay(
    ...
    amount: float = nextcord.SlashOption(
        "money-gp", "Dinero restado a tu cuenta, en gp", True, min_value=0
    ),
...
async def addmoney(
    interaction: nextcord.Interaction,
    amount: float = nextcord.SlashOption(
        "money-gp", "Dinero añadido a tu cuenta, en gp", True, min_value=0
    ),
...
async def addlanguage(
    ...
    if addedlanguage not in languages:
        ...
        sh.update_pj_data_cell(pj_row, PJ_COL.Languages,
                               [[", ".join(languages)]])
        return await interaction.send(
...
async def reputation(
    ...
    if len(reps) > 0:
        ...
        reps.sort(reverse=True,
                  key=lambda r: r[REP_COL.num(REP_COL.Reputation)])
        print(reps)
